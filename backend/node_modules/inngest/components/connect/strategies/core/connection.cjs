...
var ConnectionCore = class {
	...
	async connect(attempt = 0, path = []) {
		...
		while (true) {
			...
			try {
				...
				this.callbacks.log("Failed to connect", err instanceof Error ? err.message : err);
				...
				if (err instanceof require_util.AuthError) {
					...
					if (switchToFallback) this.callbacks.log("Switching to fallback signing key");
	...
	*/
	...
	async prepareConnection(hashedSigningKey, attempt, path = []) {
		let closed = false;
		this.callbacks.log("Preparing connection", {
			attempt,
			path
		});
		...
		if (this.config.rewriteGatewayEndpoint) {
			const rewritten = this.config.rewriteGatewayEndpoint(startResp.gatewayEndpoint);
		...
		let onConnectionError = (error) => {
			if (closed) {
				this.callbacks.log("Connection error while initializing but already in closed state, skipping", { connectionId });
				return;
			}
			closed = true;
			this.callbacks.log("Connection error in connecting state, rejecting promise", { connectionId });
			this.excludeGateways.add(startResp.gatewayGroup);
		...
		ws.onmessage = async (event) => {
			...
			this.callbacks.log(`Received message: ${require_connect.gatewayMessageTypeToJSON(connectMessage.kind)}`, { connectionId });
		...
		onConnectionError = async (error) => {
			if (closed) {
				this.callbacks.log("Connection error but already in closed state, skipping", { connectionId });
				return;
			}
			closed = true;
			await conn.cleanup();
			...
			if (currentState === require_types.ConnectionState.CLOSING || currentState === require_types.ConnectionState.CLOSED) {
				this.callbacks.log(`Connection error (${connectionId}) but already closing or closed, skipping`);
				return;
			}
			this.callbacks.onStateChange(require_types.ConnectionState.RECONNECTING);
			this.excludeGateways.add(startResp.gatewayGroup);
			if (isDraining) {
				this.callbacks.log(`Connection error (${connectionId}) but already draining, skipping`);
				return;
			}
			this.callbacks.log(`Connection error (${connectionId})`, error instanceof Error ? error.message : error);
		...
		ws.onmessage = async (event) => {
			...
			if (connectMessage.kind === require_connect.GatewayMessageType.GATEWAY_CLOSING) {
				...
				try {
					this.callbacks.log("Setting up new connection while keeping previous connection open", { connectionId });
					await this.connect(0, [...path]);
					await conn.cleanup();
				} catch (err) {
					this.callbacks.log("Failed to reconnect after receiving draining message", {
						connectionId,
			...
			if (connectMessage.kind === require_connect.GatewayMessageType.GATEWAY_EXECUTOR_REQUEST) {
				if (this.callbacks.getState() !== require_types.ConnectionState.ACTIVE) {
					this.callbacks.log("Received request while not active, skipping", { connectionId });
					return;
				}
				...
				try {
					...
					this.callbacks.log(`Execution error for request ${gatewayExecutorRequest.requestId}`, err instanceof Error ? err.message : err);
				} finally {
