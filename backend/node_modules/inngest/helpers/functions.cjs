...
const versionSchema = zod_v3.z.literal(-1).or(zod_v3.z.literal(0)).or(zod_v3.z.literal(1)).or(zod_v3.z.literal(2)).optional().transform((v) => {
	if (typeof v === "undefined") {
		console.debug(`No request version specified by executor; defaulting to v${require_InngestExecution.PREFERRED_ASYNC_EXECUTION_VERSION}`);
		return {
			sdkDecided: true,
...
const fetchAllFnData = async ({ data, api, version }) => {
	...
	const shouldFetchData = {
		[require_consts.ExecutionVersion.V0]: () => result.version === require_consts.ExecutionVersion.V0 && result.use_api,
		[require_consts.ExecutionVersion.V1]: () => result.version === require_consts.ExecutionVersion.V1 && Boolean(result.ctx?.use_api),
		[require_consts.ExecutionVersion.V2]: () => result.version === require_consts.ExecutionVersion.V2 && Boolean(result.ctx?.use_api)
	};
	try {
